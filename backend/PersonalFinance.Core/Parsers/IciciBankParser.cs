...
public class IciciBankParser : IBankStatementParser
{
    ...
    public async Task<List<BankTransaction>> ParseAsync(Stream fileStream, string fileName)
    {
        ...
        {
            throw new InvalidOperationException($"Error parsing ICICI bank statement: {ex.Message}", ex);
        }
    ...
    private List<BankTransaction> ParseExcel(Stream fileStream)
    {
        ...
        for (int i = 1; i <= sheet.LastRowNum; i++)
        {
            ...
            try
            {
                var transaction = new IciciBankTransaction
                {
                    TransactionDate = GetCellDate(row.GetCell(0)),
                    Amount = GetCellDecimal(row.GetCell(1)),
                    Description = GetCellString(row.GetCell(2)),
                    ChequeNumber = GetCellString(row.GetCell(3)),
                    Balance = GetCellDecimal(row.GetCell(4)),
                    TransactionType = GetCellDecimal(row.GetCell(1)) >= 0 ? "CR" : "DR"
                };
    ...
    private async Task<List<BankTransaction>> ParseCsv(Stream fileStream)
    {
        ...
        {
            csv.Context.RegisterClassMap<IciciTransactionMap>();
            var records = csv.GetRecords<IciciTransactionRecord>();
...
public class IciciTransactionRecord
{
...
public sealed class IciciTransactionMap : ClassMap<IciciTransactionRecord>
{
    public IciciTransactionMap()
    {
